...
//	deleteCheck 필드를 "NO"로 수정하는 category.xml 파일의 update sql 명령을 실행하는 메소드	
	public void deleteRestore(SqlSession mapper, int idx) {
		System.out.println("CategoryDAO의 deleteRestore() 메소드");
...
//	CategoryService 클래스에서 호출되는 mapper와 신고 횟수를 1증가시킬 카테고리 번호를 넘겨받고
//	카테고리 1건의 deleteReport 필드의 값을 1증가 시키는 category.xml 파일의 update sql 명령을 
//	실행하는 메소드
	public void deleteReport(SqlSession mapper, int idx) {
		System.out.println("CategoryDAO의 deleteReport() 메소드");
...
//	삭제한 카테고리 그룹에 해당되는 카테고리 목록을 얻어오는 category.xml 파일의 select sql
//	명령을 실행하는 메소드
...
//	CategoryService 클래스에서 호출되는 mapper와 seq를 수정할 카테로기 일련번호와 수정할 데이터가
