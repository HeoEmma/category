...
//	deleteRestore.jsp에서 호출되는 복구할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 클래스의
//	카테고리 1건의 deleteCheck 필드의 값을 "NO"로 수정하는 update sql 명령을 실행하는 메소드를 
//	호출하는 메소드
	public void deleteRestore(int idx) {
		System.out.println("CategoryService의 deleteRestore() 메소드");
		SqlSession mapper = MySession.getSession();
...
//	update.jsp에서 호출되는 수정할 카테고리 정보가 저정된 객체를 넘겨받고 mapper를 얻어온 후 
...
//	카테고리 1건의 deleteReport 필드의 값을 1증가(수정) 시키는 update sql 명령을 실행하는 메소드를 
//	호출하는 메소드
	public void deleteReport(int idx) {
		System.out.println("CategoryService의 deleteReport() 메소드");
		SqlSession mapper = MySession.getSession();
...
//	호출하는 메소드
	public ArrayList<CategoryVO> deleteList(CategoryVO vo) {
		System.out.println("CategoryService의 deleteList() 메소드");
		SqlSession mapper = MySession.getSession();
